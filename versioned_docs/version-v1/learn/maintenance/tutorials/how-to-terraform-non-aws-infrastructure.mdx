...
Most of the infrastructure found inside of the infrastructure repository is for AWS, but companies rely on other platforms like Datadog, Snowflake, etc. If the `infrastructure` repository is just for AWS, then where do we put the rest of the infrastructure code for these systems?
...
- You could also host the infrastructure code in a separate repository and point spacelift to that repository. See [How to Use Multiple Infrastructure Repositories with Spacelift?](/resources/deprecated/spacelift/tutorials/how-to-use-multiple-infrastructure-repositories-with-spacelift)
