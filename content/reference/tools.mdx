...
The `build-harness` is a collection of [Makefiles](/reference/tools.mdx#make) to facilitate building stuff. It supports Golang projects, Dockerfiles, Helm charts, and much more.
...
It's 100% Open Source and licensed under [APACHE2](LICENSE.md).
...
We leverage the `build-harness` in nearly every project on our [GitHub](/our-github.md).
...
Using chamber you can perform all standard CRUD operations on secrets stored in [AWS SSM](/glossary/ssm.md) and execute commands with environment variables populated from the secrets.
...
- **Chamber Compatibility** - Use [`chamber`](/reference/tools.mdx#chamber) to deploy helm charts with secrets pulled from SSM
- **CI/CD Integration** - Easily integrate `helmfile` with CI/CD pipelines using our [`build-harness`](/reference/tools.mdx#build-harness)
...
- [`helm`](/reference/tools.mdx#helm) - for managing helm packages
- [`kubectl`](/reference/tools.mdx#kubectl) - for interfacing with the Kubernetes API
...
1. [Our Terraform Best Practices](/reference/best-practices/terraform-best-practices.md)
1. [Our Terraform Modules](https://github.com/cloudposse?q=terraform-)
1. [Our Terraform Tips & Tricks](/reference/best-practices/terraform-tips-tricks.md)
