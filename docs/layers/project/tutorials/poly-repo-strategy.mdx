...
### Are Rego OPA policies used? If so, are they stored in the infrastructure repo and downloaded into the service repo?

At the moment, we don't have a solution for this.

### How do you keep the binaries the same version across entities?

This is currently a challenge that needs to be addressed based on your specific requirements. Consider using a centralized version management approach or CI/CD pipeline that ensures consistent versions.
...
<ActionCard title="What comes next?">
  Now that you understand how to manage multiple repositories with `account-map`, you can apply this pattern to your infrastructure projects. Consider exploring our other tutorials to learn more about infrastructure management with Atmos and Terraform.

  <PrimaryCTA to="/layers/project/tutorials">View More Tutorials</PrimaryCTA>
</ActionCard>

<Note>
  This tutorial is based on a [GitHub discussion](https://github.com/orgs/cloudposse/discussions/49) about splitting up YAML and Terraform into service repos.
</Note> 