...
OpenTofu is based on Terraform 1.5.7 (the last MPL-licensed version) and maintains compatibility with Terraform 1.x modules and continues to evolve as a fully open-source project under the CNCF. Cloud Posse modules and components are verified to work with OpenTofu, but with hundreds of modules, there may be delays in verifying full support with every new release.
