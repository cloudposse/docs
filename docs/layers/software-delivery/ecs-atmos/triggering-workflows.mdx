...
<Admonition type="info" title="Cluster Selection">
  The GitHub workflow specifies which Atmos stack to deploy with the `-s` flag (e.g., `atmos terraform deploy app -s staging`). Each stack configuration file defines which ECS cluster and account to target. This explicit stack selection in the workflow ensures the right environment is deployed at each stage.
</Admonition>
