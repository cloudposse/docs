  ...
  <Step>
    ...
    <Steps>
      ...
      2. Upload these credentials to AWS SSM Parameter Store
          <TaskList>
          - Upload the `Argo CD Instance` private key to `/argocd/argo_cd_instance/app_private_key` `core-auto`: 
            ```bash
            ...
            ```
          - Upload the `Argo CD Notifications` to `/argocd/argo_cd_notifications/app_private_key` to `core-auto`:
            ```bash
            assume-role acme-core-gbl-auto-admin
