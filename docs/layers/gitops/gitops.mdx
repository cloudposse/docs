...
- [`gitops/s3-bucket`](https://github.com/cloudposse-terraform-components/aws-s3-bucket): Deploy a S3 Bucket using the `s3-bucket` component. This bucket holds Terraform planfiles.
- [`gitops/dynamodb`](https://github.com/cloudposse-terraform-components/aws-dynamodb): Deploy a DynamoDB table using the `dynamodb` component. This table is used to hold metadata for Terraform Plans
- [`github-oidc-role`](https://github.com/cloudposse-terraform-components/aws-github-oidc-role): Deploys an IAM Role that GitHub is able to assume via GitHub OIDC. This role has access to the bucket and table for planfiles.
