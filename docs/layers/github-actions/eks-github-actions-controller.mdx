...
<Step>
  ...
  Deploy the cluster with the same commands as `plat`, but then deploy `eks/actions-runner-controller`.

  ```bash
  atmos workflow deploy/iam-service-linked-roles -f eks -s core-use1-auto
