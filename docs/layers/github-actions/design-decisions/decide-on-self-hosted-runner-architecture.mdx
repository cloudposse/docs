...
<Intro>
  Self-hosted runners are custom runners that we use to run GitHub Actions workflows. We can create these runners a
  number of ways.
</Intro>

## Problem

We support multiple options for deploying self-hosted runners. We can deploy runners with EKS, Philips Labs, or with an
ASG. For this decision, we will focus on the placement of the runners themselves.
...
<Steps>
  <Step>
    #### <StepNumber /> Limited set of packages
  ...
  <Step>
    #### <StepNumber /> Docker in Docker (dind) mode only
  ...
  <Step>
    #### <StepNumber /> Limited configuration options
  ...
  <Step>
    #### <StepNumber /> Component limitations
