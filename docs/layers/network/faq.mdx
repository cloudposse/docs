...
For users needing to deploy “global” components to a specific AWS region, you can set `overridable_global_environment_name` to the desired environment (e.g., `use1`), which allows all “global” components to be deployed consistently without creating regional conflicts.
