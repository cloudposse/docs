  ...
  <Step>
    ...
    <Tabs queryString="platform">
      ...
      <TabItem value="eks" label="Use EKS">
        ...
        Amazon EKS is a managed Kubernetes service that allows you to run Kubernetes in AWS cloud and on-premises data centers. AWS handles the availability and scalability of the Kubernetes control plane, which oversees tasks such as scheduling containers, managing application availability, and storing cluster data. With [EKS Auto Mode](https://docs.aws.amazon.com/eks/latest/userguide/automode.html), AWS also manages compute, networking, and storage — including node provisioning via Karpenter and load balancer management. Additional Karpenter node pools can be configured for custom compute needs like GPU instances or spot optimization. Be aware that you'll need to upgrade the cluster quarterly due to the significant pace of Kubernetes innovation. Although EKS has a steeper learning curve compared to ECS, it offers greater flexibility and control, making it ideal for organizations already utilizing Kubernetes.
