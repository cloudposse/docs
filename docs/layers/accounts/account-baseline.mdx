...
<Steps>
  ...
  Budgets are an optional feature that can be enabled with [the `account-settings` component](https://github.com/cloudposse-terraform-components/aws-account-settings) for the Organization as a whole or for individual accounts. Budgets *do not restrict spending* but provide visibility into spending and can be used to set alerts when spending exceeds a certain threshold. We recommend using a dedicated Slack channel for these alerts, which we will set up with a webhook.
