...
<Steps>
  ...
  <Step>
    ...
    **root** account:
    ...
    `arn:aws:iam::*:role/acme-*-gbl-*-terraform`) in the `tfstate-backend` catalog defaults instead. This trades
    security for convenience and avoids the need for a quota increase.
    :::
