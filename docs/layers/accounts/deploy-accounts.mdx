...
<Steps>
  ...
  <Step>
    ...
    1. Update account IDs in your stack configuration files. Common locations include:
       <Steps>
       1. `stacks/orgs/<namespace>/_defaults.yaml` — The `account_map` variable
       1. `stacks/catalog/tfstate-backend.yaml` — The `allowed_principal_arns` if referencing specific accounts
       1. Any component configurations referencing account IDs directly
       </Steps>
