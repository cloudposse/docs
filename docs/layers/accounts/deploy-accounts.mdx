...
- Remove Step 4 "Confirm Root Account Name" (account-map specific)
- Remove the Note in Step 6 about dynamic Terraform roles and account-map
- Update Step 8 "ClickOps to Complete Account Setup" - root password reset is no longer needed with centralized root access
- Add sub-page or section for setting up RootAccess permission set as alternative to per-account root credentials
*/}
