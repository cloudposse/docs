...
<Steps>
  ...
  <Step>
    ...
    Create the necessary stack configuration files for your new account using the `stacks/orgs/<namespace>/<tenant>/<stage>` pattern. See the following example where the namespace is `acme`, the tenant is `core`, and the stage is `foobar`.
