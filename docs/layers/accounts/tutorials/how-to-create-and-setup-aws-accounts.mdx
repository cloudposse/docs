...
<Steps>
  <Step>
    ...
    **Example for a new account named `pca`:**
  ...
  <Step>
    ...
    Create the necessary stack configuration files for your new account using the modern `stacks/orgs/namespace/tenant/region` pattern. You'll typically need a global-region stack for account settings, and regional stacks depending on your specific use case.

    **Stage Mixin** (`stacks/mixins/stage/pca.yaml`):
    ...
    **Account Defaults** (`stacks/orgs/acme/core/pca/_defaults.yaml`):
    ...
    **Account Settings** (`stacks/orgs/acme/core/pca/global-region/baseline.yaml`):

    This file configures account-level settings and policies.

    ```yaml
    import:
      - orgs/acme/core/pca/_defaults
    ...
    **AWS Team Roles** (`stacks/orgs/acme/core/pca/global-region/identity.yaml`):
    ...
    **Regional Stack** (`stacks/orgs/acme/core/pca/us-east-1/baseline.yaml`):
    ...
    **Example Directory Structure:**
    ...
    For a new account `pca` in the `core` tenant under namespace `acme`:

    ```
    stacks/orgs/acme/core/pca/
    ├── _defaults.yaml
    ├── global-region/
  ...
  <Step>
    ...
    **Deploy Components:**
    ...
    atmos terraform apply account-settings --stack core-gbl-pca

    # Deploy AWS Team Roles to enable cross-account access
    atmos terraform apply aws-team-roles --stack core-gbl-pca
    ```

    <Note title="Component Order">
      The order of deployment is important. The `account` component creates the account, `account-map` updates the account mapping, `account-settings` configures the account, and `aws-team-roles` enables cross-account role assumption.
    </Note>
  </Step>
  ...
  <Step>
    ...
    **Quick Steps:**
    <Steps>
    1. Reset password by logging into AWS console with the account's email address and clicking "Forgot password?"
    2. Set up MFA using a Virtual MFA device (save the TOTP key in 1Password)
    3. Enable required regions in the Billing console if needed
    4. Unsubscribe from AWS marketing emails at [AWS Marketing Preferences](https://pages.awscloud.com/communication-preferences.html)
    </Steps>

    <Note title="Important">
      Save the root credentials, MFA TOTP key, and account number in 1Password for future reference.
    </Note>
