...
flowchart TB
    subgraph org["AWS Organization"]
        subgraph root["Organization Management Account (root)"]
            step1["STEP 1: Delegate Administration<br/>Creates: aws_organizations_delegated_administrator<br/>Creates: aws_iam_service_linked_role"]
        end

        subgraph security["Delegated Administrator Account (security)"]
            step2["STEP 2: Create Organization Analyzers"]
            subgraph dashboard["Access Analyzer Dashboard"]
                external["External Access<br/>Findings"]
                unused["Unused Access<br/>Findings"]
            end
        end

        subgraph members["Member Accounts"]
            member["Auto-analyzed resources<br/>No per-account config needed<br/>Findings sent to security"]
        end
    end

    step1 -->|"Delegation"| security
    member -->|"Resource Analysis"| dashboard
```
...
```bash
atmos terraform apply access-analyzer/root -s core-gbl-root
```
...
components:
  terraform:
    access-analyzer/delegated-administrator/ue1:
      metadata:
        component: access-analyzer
      vars:
        enabled: true
...
```bash
atmos terraform apply access-analyzer/delegated-administrator/ue1 -s core-ue1-security
```
...
atmos terraform apply access-analyzer/delegated-administrator/ue1 -s core-ue1-security
atmos terraform apply access-analyzer/delegated-administrator/uw2 -s core-uw2-security
```
