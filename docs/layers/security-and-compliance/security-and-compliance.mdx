...
```mermaid
%%{init: {'flowchart': {'padding': 40, 'nodeSpacing': 30, 'rankSpacing': 50}}}%%
flowchart TB
    subgraph org["AWS Organization"]
        subgraph root["Root Account"]
            root_tasks["• Delegates administration<br/>• Enables org service access"]
        end

        subgraph security["Security Account (Delegated Admin)"]
            sec_services["• GuardDuty<br/>• Security Hub<br/>• Inspector<br/>• Macie<br/>• Access Analyzer"]
        end

        subgraph audit["Audit Account"]
            audit_storage["• CloudTrail logs bucket<br/>• Config snapshots"]
        end

        subgraph members["Member Accounts (prod, staging, dev)"]
            member_info["• Auto-enrolled<br/>• Monitored by security"]
        end
    end
