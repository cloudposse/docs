...
│                                                                             │
│  ┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐      │
│  │   Root Account   │    │  Security Account │    │   Audit Account  │      │
│  │                  │    │ (Delegated Admin) │    │                  │      │
│  │ • Delegates      │───▶│ • GuardDuty      │    │ • CloudTrail     │      │
│  │   administration │    │ • Security Hub   │    │   logs bucket    │      │
│  │ • Enables org    │    │ • Inspector      │    │ • Config         │      │
│  │   service access │    │ • Macie          │    │   snapshots      │      │
│  │                  │    │ • Access Analyzer│    │                  │      │
│  └──────────────────┘    └────────┬─────────┘    └──────────────────┘      │
│                                   │                                         │
│                    ┌──────────────┴──────────────┐                         │
...
│  │  (prod, staging, dev)    │  │    (workload accounts)   │                │
│  │  • Auto-enrolled         │  │  • Findings → Security   │                │
│  │  • Monitored by security │  │  • Logs → Audit          │                │
│  └──────────────────────────┘  └──────────────────────────┘                │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| [AWS Inspector](/layers/security-and-compliance/aws-inspector2/) | Automated vulnerability scanning | 2-step delegated admin |
...
[**AWS Inspector**](/layers/security-and-compliance/aws-inspector2/) provides automated vulnerability scanning for
...
```text
Step 1: Security Account → Create service account (admin_delegated: false)
Step 2: Root Account     → Delegate administration (privileged: true)
Step 3: Security Account → Configure org settings (admin_delegated: true)
```
...
```text
Step 1: Root Account     → Delegate administration (privileged: true)
Step 2: Security Account → Configure org settings (admin_delegated: true)
```
...
```text
CloudTrail ──────────────────────────────────▶ Audit Account S3
Config ─────▶ Snapshots ─────────────────────▶ Audit Account S3 + Aggregator
                                                     │
GuardDuty ─────▶ Findings ──────────────────┐       │
...
2. **Any additional regions** you have opted into

## Getting Started

1. Review the [Setup Guide](/layers/security-and-compliance/setup/) for step-by-step deployment instructions
2. Explore individual component documentation for detailed configuration options
3. Check the [Design Decisions](/layers/security-and-compliance/design-decisions/) for architectural considerations
4. Review the [FAQ](/layers/security-and-compliance/faq/) for common issues and solutions
