...
Before deploying security and compliance components, ensure:

1. **AWS Organizations** is configured with your account structure
2. **Account baseline** has been deployed (see [Account Baseline](/layers/accounts/account-baseline/))
3. **SuperAdmin user** is available for root account deployments (see [How to Create SuperAdmin](/layers/accounts/tutorials/how-to-create-superadmin-user/))
...
│  ├── Service Principals (account component)                                 │
│  ├── config-bucket (audit account)                                          │
│  └── cloudtrail-bucket (audit account)                                      │
│                                                                             │
│  Phase 2: Foundational Services                                             │
│  ├── AWS CloudTrail (organization trail)                                    │
│  └── AWS Config (all accounts, all regions)                                 │
│                                                                             │
│  Phase 3: Threat Detection & Vulnerability Scanning                         │
│  ├── AWS GuardDuty (3-step: security → root → security)                     │
│  ├── AWS Inspector (2-step: root → security)                                │
│  ├── Amazon Macie (3-step: security → root → security)                      │
│  └── IAM Access Analyzer (2-step: root → security)                          │
│                                                                             │
│  Phase 4: Aggregation & Protection                                          │
│  ├── AWS Security Hub (3-step: security → root → security)                  │
│  ├── AWS Shield (per-resource, where needed)                                │
│  └── AWS Audit Manager (optional, root account only)                        │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
<Steps>
  ...
  <Step>
    ...
    <Note title="Reminder">
      This requires [SuperAdmin](/layers/accounts/tutorials/how-to-create-superadmin-user/). Ensure the `plan` output
  ...
  <Step>
    ...
    [Account Baseline](/layers/accounts/account-baseline/).
    </Note>
...
<Steps>
  <Step>
    ...
    <AtmosWorkflow workflow="deploy/aws-cloudtrail" fileName="compliance" />
...
<Steps>
  <Step>
    ...
    <Note title="Reminder">
      This requires [SuperAdmin](/layers/accounts/tutorials/how-to-create-superadmin-user/) for root account deployments.
    </Note>
  </Step>

  <Step>
    ### <StepNumber/> Deploy AWS Config for SuperAdmin Accounts

    Deploy AWS Config to accounts that require SuperAdmin to apply (root, security).
...
<Steps>
  ...
  <Step>
    ...
    <Note title="Reminder">
      This requires [SuperAdmin](/layers/accounts/tutorials/how-to-create-superadmin-user/).
    </Note>
...
[AWS Inspector](/layers/security-and-compliance/aws-inspector2/) provides automated vulnerability scanning for EC2,
...
<Steps>
  <Step>
    ...
    <AtmosWorkflow workflow="deploy/macie/step1" fileName="compliance" />
  </Step>
  ...
  <Step>
    ...
    <AtmosWorkflow workflow="deploy/macie/step2" fileName="compliance" />
  </Step>
  ...
  <Step>
    ...
    <AtmosWorkflow workflow="deploy/macie/step3" fileName="compliance" />
  </Step>
</Steps>
...
<Steps>
  <Step>
    ...
    <AtmosWorkflow workflow="deploy/access-analyzer/step1" fileName="compliance" />
  </Step>
  ...
  <Step>
    ...
    <AtmosWorkflow workflow="deploy/access-analyzer/step2" fileName="compliance" />
  </Step>
</Steps>
...
<Steps>
  ...
  <Step>
    ...
    Switch from SuperAdmin back to your identity role:
...
<Steps>
  <Step>
    ...
    Or deploy manually to specific stacks:
...
3. Verify findings are being aggregated from all services
...
2. Navigate to "Security standards"
3. Review compliance scores for enabled standards (CIS, PCI DSS, AWS Foundational)
