...
flowchart TB
    subgraph org["AWS Organization"]
        subgraph root["Organization Management Account (root)"]
            single_step["SINGLE STEP: Enable + Delegate<br/>Requires: SuperAdmin (privileged: true)"]
        end

        subgraph security["Delegated Administrator Account (security)"]
            admin_info["Receives delegation automatically<br/>Creates and manages assessments"]
            subgraph dashboard["Audit Manager Dashboard"]
        ...
        subgraph members["Member Accounts"]
            member["Auto-collected evidence<br/>CloudTrail, Config, Security Hub"]
        end
    end

    single_step -->|"Automatic Delegation"| security
    member -->|"Evidence Collection"| dashboard
```
