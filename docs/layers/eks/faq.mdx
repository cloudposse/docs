...
This FAQ entry applies to deployments using the self-managed `alb-controller` and `alb-controller-ingress-group`
components. For new deployments, [EKS Auto Mode](/layers/eks/foundational-platform/) manages load balancers
automatically, and the [`eks/ingress-class`](/components/library/aws/eks/alb-controller-ingress-class) component
replaces `alb-controller-ingress-group`.
