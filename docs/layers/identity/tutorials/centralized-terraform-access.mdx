...
:::warning Deprecated
This documentation describes the legacy architecture using **`aws-teams`** and **`aws-team-roles`** components for centralized access management.

**The recommended approach now uses:**
- **Permission Sets only** (TerraformPlanAccess, TerraformApplyAccess, TerraformStateAccess) for human user access
- **`iam-role` component** for machine users (CI/CD) with GitHub OIDC
- **Static `account_map` variable** instead of dynamic account-map component
- **Identity managed in `core-root`** instead of separate `core-identity` account

See [Access Control Evolution](/layers/identity/docs/aws-access-control-evolution) for the current recommended approach.
:::
