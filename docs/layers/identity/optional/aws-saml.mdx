...
:::warning Deprecated
This documentation describes using **AWS SAML** for federated identity access, which is an advanced option that provides lower-level control but with more complexity.

**The recommended approach now uses:**
- **AWS IAM Identity Center** (AWS SSO) for user authentication
- **Permission Sets** (TerraformPlanAccess, TerraformApplyAccess, TerraformStateAccess) for access control
- **Atmos Auth** via `atmos auth login` for CLI authentication

This approach is preserved for organizations with specific requirements for direct SAML federation.
:::
