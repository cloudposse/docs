...
:::warning Deprecated
This documentation describes the legacy Dynamic Terraform Roles feature using `aws-teams` and `aws-team-roles` components.

**The recommended approach now uses:**
- **Permission Sets** (TerraformPlanAccess, TerraformApplyAccess, TerraformStateAccess) for human users
- **`iam-role` component** for machine users (CI/CD)
- **Static `account_map` variable** instead of dynamic lookups

See [Migrate from Account-Map](/layers/identity/tutorials/migrate-from-account-map) for migration instructions.
:::
