...
```bash
# List pods in the dev EKS cluster
atmos auth exec --identity plat-dev/terraform -- kubectl get pods -A

# Check node status
atmos auth exec --identity plat-dev/terraform -- kubectl get nodes

# View cluster info for staging
atmos auth exec --identity plat-staging/terraform -- kubectl cluster-info
```

:::note
EKS cluster access requires a VPN connection to the target account. See the
[EKS deployment guide](/layers/eks/deploy-clusters/) for details.
:::
