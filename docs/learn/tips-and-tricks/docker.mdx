...
Docker doesn't technically support multiple-inheritance, whereby an image can automatically merge multiple images using `FROM`. It does, however, support [multi-stage builds](/best-practices/docker#multi-stage-builds) that can be used to effectiely achive the same result.
