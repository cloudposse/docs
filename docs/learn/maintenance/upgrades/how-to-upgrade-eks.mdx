...
We need to install the version of `kubectl` that is at least one minor version away from the `cluster_kubernetes_version` in [Geodesic](/reference/legacy/fundamentals/geodesic). Installation instructions will vary depending on if you’re using Alpine, Ubuntu, or CentOS.  For the most current list of `kubectl-1.x` versions, check out our `cloudposse/packages` repository [https://github.com/cloudposse/packages/tree/master/vendor](https://github.com/cloudposse/packages/tree/master/vendor).
