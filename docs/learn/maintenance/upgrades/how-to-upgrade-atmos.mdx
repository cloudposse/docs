...
<Note title="TL;DR">
  Update the `ATMOS_VERSION` in the [Geodesic](/reference/legacy/fundamentals/geodesic) `Dockerfile` of your `infrastructure` repository.
</Note>
...
<Steps>
  1. Update the `ATMOS_VERSION` in the [Geodesic](/reference/legacy/fundamentals/geodesic) `Dockerfile` of your `infrastructure` repository (see “Atmos Versions”)
  2. Remove `variant` installation from the Dockerfile

  3. Add `atmos.yaml` to  `rootfs/usr/local/etc/atmos/atmos.yaml`
      - See [Atmos](/reference/legacy/fundamentals/atmos)  for example.
