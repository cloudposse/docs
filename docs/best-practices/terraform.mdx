...
The AWS provider updates frequently and it's challenging to stay up to date with all of its attributes. While it is still important to have an explicit output for necessary attributes, there is a limbo period where those outputs haven't been added yet to a module. For that reason, exposing all attributes of a resource is possible with this convention.
