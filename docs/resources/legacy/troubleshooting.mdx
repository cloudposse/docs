...
This error is most likely due to a security group issue. The [transit-gateway](https://github.com/cloudposse-terraform-components/aws-transit-gateway) component controls ingress from `auto` and `corp` into the other accounts. Check the `transit-gateway`'s input var `accounts_with_eks` is filled in correctly and redeploy the component.
