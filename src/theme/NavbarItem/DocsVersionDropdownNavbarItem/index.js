 ...
 *
 * The fix: detect version from URL path and use that for the display label,
 * since useDocsVersionCandidates() returns the "preferred" version instead
 * of the actually viewed version.
 *
 * Updated for Docusaurus 3.9.2 API.
 *
...
// FIX: Detect version from URL path since useDocsVersionCandidates is unreliable
function useUrlDetectedVersionItem({versionItems}) {
  const pathname = useHistorySelector((history) => history.location.pathname);
  const isV1Path = pathname.startsWith('/v1/') || pathname === '/v1';

  if (isV1Path) {
    const v1Item = versionItems.find((vi) => vi.version.name === 'v1');
    if (v1Item) return v1Item;
  }

  // Default to current version
  const currentItem = versionItems.find((vi) => vi.version.name === 'current');
  return currentItem ?? versionItems[0];
}
...
export default function DocsVersionDropdownNavbarItem({
  ...
  // Original: uses useDisplayedVersionItem which relies on useDocsVersionCandidates
  // const displayedVersionItem = useDisplayedVersionItem({ docsPluginId, versionItems });

  // FIX: Use URL-detected version instead
  const displayedVersionItem = useUrlDetectedVersionItem({versionItems});
