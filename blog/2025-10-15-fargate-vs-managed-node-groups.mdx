...
By contrast, a managed node group for your baseline cluster infrastructure gives you flexibility:

<FeatureList>
  - Use Graviton-based instances (c7g.medium) for the MNG to cut costs nearly in half
  - Run the static MNG with On-Demand instances for reliability
  - Let Karpenter provision Spot instances for dynamic workloads, maximizing cost savings
  - Keep the baseline small and predictable while Karpenter handles burst capacity
</FeatureList>

The result: **reliable foundation with On-Demand nodes, cost-efficient scaling with Spot**.
